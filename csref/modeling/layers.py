...
class AttFlat(nn.Module):
    def __init__(self, hidden_size: int, flat_glimpses: int, dropout_rate: float):
...
class MHAtt(nn.Module):
    ...
    def forward(self, v: torch.Tensor, k: torch.Tensor, q: torch.Tensor, mask: Optional[torch.Tensor] = None) -> torch.Tensor:
        n_batches = q.size(0)

        v = self.linear_v(v).view(
            n_batches, -1, self.num_heads, int(self.hidden_size / self.num_heads)
        ).transpose(1, 2)
    ...
    def att(self, value: torch.Tensor, key: torch.Tensor, query: torch.Tensor, mask: Optional[torch.Tensor] = None) -> torch.Tensor:
        d_k = query.size(-1)
...
class SA(nn.Module):
    def __init__(self, hidden_size: int, num_heads: int, ffn_size: int, dropout_rate: float):
    ...
    def forward(self, y: torch.Tensor, y_mask: torch.Tensor, pos: Optional[torch.Tensor] = None) -> torch.Tensor:
...
class SGA(nn.Module):
    ...
    def forward(self, x: torch.Tensor, y: torch.Tensor, x_mask: torch.Tensor, y_mask: torch.Tensor,
                q_pos: Optional[torch.Tensor] = None, k_pos: Optional[torch.Tensor] = None) -> torch.Tensor:
